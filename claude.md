...
- **Above 1920px there are two families of utilities**, both taking `X` = the element's exact px value in the Figma 1920 frame, both used behind the `uw:` variant:
  ...
  - **`cap-*` is the default for everything** — type, spacing, sizes, positioning, all of it. Uncapped growth gets absurd on a 4K screen (type especially). Only reach for `uw-*` when the user explicitly asks for it on a given element.
...
- **Collapse redundant classes**: `cap-px-126`, never `cap-pl-126 cap-pr-126`.
