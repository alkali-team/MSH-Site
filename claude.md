...
- **Two families of utilities**, both taking `X` = the element's exact px value in the Figma 1920 frame:
  - **`uw-*` — grows forever**, but not linearly. Up to 1920 it is plain proportional `vw`; above 1920 it keeps only `--uw-grow` (0.8) of the growth, because the linear curve overshot the 3840 frame on nearly every element. A 57px element is 57px at 1920 and 102.6px at 3840, not 114px. The two lines cross exactly at 1920, so `min()` picks the right one on each side — no media query, no seam, and nothing below 1920 changes. `--uw-grow` is the knob; `1` restores plain linear.
  - **`cap-*` — plain linear growth, frozen at `--uw-cap` (2600px)**. `cap-px-122` → 122px at 1920, 165.2px at 2600, still 165.2px at 3840. It deliberately does **not** take `--uw-grow`; it stays on the straight 1920 line and stops. Known consequence, signed off — between 1920 and ~2770 a `cap-*` value comes out up to ~5% larger than the same number in `uw-*`, because `uw-*` flattens there and this doesn't. Don't "fix" it by deriving `cap-*` from `--uw-unit`. There is no `@media` breakpoint at the cap; it falls out of a `min()` between the `vw` term and an `X * cap / 1920` ceiling.
  ...
  - **`uw-*` is the default. Reach for `cap-*` only with a specific reason.** Two earlier versions of this rule said the opposite — first that `cap-*` was the default for everything, then that type was the one exception. Both were wrong against the actual design: checked element by element, nearly everything sits far closer to plain proportional growth than to a value frozen at 2600. Capping is the special case, not the norm. Don't reintroduce a `cap-*`-first rule.
  - **A `cap-*` value is only safe where an `xw:` rule takes over above 2560.** It freezes at 2600, so with no 3840 frame to step in, that element stays small on a 4K screen while everything around it keeps growing. Most pages have a 1920 frame and nothing else, so on those `cap-*` has no legitimate use at all.
...
- A custom faster-than-linear growth formula was tried and **rejected** — don't reintroduce it as the default. It was fit from two guessed/eyeballed target sizes (not real Figma data): 57px→135.65px (1920→3840) for one element implied a ~2.38x growth factor at 3840, but a second element's guessed target (1434px→2721px) implied a DIFFERENT, slower-than-linear factor (~1.9x) — so it was dropped. Note the curve that eventually stuck goes the *other* way: `--uw-grow: 0.8` yields 1.8x at 3840, within a few percent of that second data point (~1.9x). A usable universal factor does exist — it is slower than linear, not faster.
...
- **Every section uses the same 100px horizontal padding** (`uw-px-100`) — don't read it off each Figma frame. The frames vary (100, 122, 126, 196 across four sections) because they were laid out by hand; the site needs one consistent gutter.
- **Collapse redundant classes**: `uw-px-100`, never `uw-pl-100 uw-pr-100`.
